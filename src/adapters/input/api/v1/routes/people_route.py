...
from src.adapters.input.api.v1.dtos.people_dto import PeopleRequestDTO, PeopleResponseDTO
...
class peopleRouter:
      ...
      async def add_people(
            ...
            password = salt + ':' + PasswordService().hash_password(params.password, salt)
            AuditLogger().log_info(f"{people}", "add_people")
            created_people = await use_case.add_people(people.name, people.group_id, people.gift_suggestion, people.email, password)
