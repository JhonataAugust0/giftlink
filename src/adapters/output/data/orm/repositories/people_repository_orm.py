...
from src.adapters.output.log.audit_logger import AuditLogger
...
from src.adapters.output.data.orm.entities.orm_entities import Grupos, Pessoa
...
class PeopleRepositoryORM(PeopleRepository):
    ...
    async def add_people_in_group(self, people: People, group_id: int) -> Pessoa:
        ...
        people_orm = Pessoa(
            nome=people.name,
            sugestao_presente=people.gift_suggestion,
            grupo_id=group_id,
            codigo=randint(1000, 9999),
            email=people.email,
            senha=people.hashed_password,
    ...
    async def get_people_by_email(self, email: str) -> Pessoa:
        """Realiza um SELECT por email."""
        result = await self.session.execute(select(Pessoa).where(Pessoa.email == email, Pessoa.ativa == True))
        pessoa = result.scalars().first()
        if not pessoa:
            raise NoResultFound(f"Pessoa com email {email} não encontrada.")
        return pessoa
